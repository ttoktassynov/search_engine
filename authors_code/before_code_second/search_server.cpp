...
void SearchServer::AddQueriesStream(istream& query_input, ostream& search_results_output)
{
  ...
  //vector<string> query_strings = {istream_iterator<string>(query_input), istream_iterator<string>()};
  query_strings.reserve(MAX_QUERY);
  for (string current_query; getline(query_input, current_query); ) {
        query_strings.push_back(move(current_query));
  }
  size_t page_size = ceil(static_cast<double>(query_strings.size()) / 4);
  
  //starts here 
  vector< vector<string>::iterator> query_its;
  auto it = query_strings.begin();
  query_its.push_back(it);
  for (size_t i = 0; i < page_size - 1; ++i)
  {
    it = it + step;
    query_its.push_back(it);
  }
  query_its.push_back(end(query_strings));
  //ends here

  auto pages = Paginate(query_strings, page_size);
  for (auto page : pages)
  {
    futures.push_back(
      async(&SearchServer::AddQueriesStreamThread, this, page, ref(index), ref(search_results_output))
    );
  }*/
  futures.push_back(
